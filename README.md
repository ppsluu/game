...
## Особенности реализации (как ленивый студент):

1. **Короткие названия**:
   - `P` вместо `Player`
   - `ABox` вместо `ArtifactsBox`
   - `n`, `lvl`, `hp`, `art`, `prog`, `branch` вместо полных названий

2. **Простые конструкции**:
   - Все выборы через `if/elif`
   - Минимум классов
   - Простая структура данных

3. **Память игры**:
   - Сохранение прогресса в JSON
   - Сохранение состояния артефактов
   - Загрузка при входе

4. **Система артефактов**:
   - Копилка хранит доступные артефакты
   - При возврате артефакты идут обратно в копилку
   - Автогенерация при полном сборе

5. **Безопасность** (как у ленивого студента):
   - Логин/пароль в открытом виде в `users.txt`
   - Формат: `логин:пароль` (одна строка = один пользователь)
